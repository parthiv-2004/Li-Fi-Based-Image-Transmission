...
binary_data = image_to_binary("black_face.png")

# Print the binary array before transmission
print("Binary Array (Transmitter):")
print(binary_data.reshape(32, 32))
...
# Send start sequence
for index, bit in enumerate(start_sequence, start=1):
    arduino.write(bit.encode())
    print(f"Sent (Start Sequence) - Index {index}: {bit}")
    time.sleep(1)

# Send binary data
for index, bit in enumerate(binary_data, start=1):
    arduino.write(str(bit).encode())
    print(f"Sent (Data) - Index {index}: {bit}")
    time.sleep(1)
